...
def train_mini_sae(block):
    ...
    sae = train_sae(stream, hidden, epochs, batches, 0.001, 
                    dataloader=dataloader, llm=sample_llm, block=block, checkpoint_path="small_sae/layer" + str(block) + ".pt", time_limit=600)
...
if __name__ == "__main__":
    ...
    for layer in range(13):
        ...
        model = load_model("small_sae/layer" 
                           + str(layer) + ".pt")

        tokens, animal, breed, activations = generate_activations(model, sample_llm, 1000, block=layer)
